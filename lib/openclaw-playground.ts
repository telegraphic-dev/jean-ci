...
export async function runGatewayPlaygroundProbe(
  ...
  const sendResult = await gatewayRpc('chat.send', {
    sessionKey,
  ...
  const historyResult = await gatewayRpc('chat.history', {
    sessionKey,
    limit: 10,
  }, {
    role: privileges.role,
  ...
  return {
    ...
    ...(historyResult.success
      ...
      : { error: historyResult.error, errorDetails: historyResult.errorDetails }),
  };
}
