...
test('websocket LLM path keeps waiting across agent.wait timeouts before succeeding', async () => {
  const calls: Array<{ method: string; params: Record<string, unknown> }> = [];
  const userMessage = 'Review this PR';
  const metadata = { owner: 'telegraphic-dev', repo: 'jean-ci', prNumber: 113, promptName: 'slow review.md' };
  const requestedKey = 'jean-ci:telegraphic-dev:jean-ci:113:slow-review-md';
  const expectedKey = 'agent:main:jean-ci:telegraphic-dev:jean-ci:113:slow-review-md';
  let waitCalls = 0;
...
test('websocket LLM path stops retrying the whole review when the same run exhausts the wait budget', async () => {
  let waitCalls = 0;
  const requestedKey = 'jean-ci:telegraphic-dev:jean-ci:117:very-slow-review-md';
  const expectedKey = 'agent:main:jean-ci:telegraphic-dev:jean-ci:117:very-slow-review-md';
...
test('websocket LLM path can target a non-main agent', async () => {
  ...
  const result = await callOpenClawResponsesViaWebSocketForTest('Review this PR', metadata, async (method, params) => {
    ...
    if (method === 'sessions.send') {
      return { success: true, result: { runId: 'run-qa', status: 'accepted', messageSeq: 1 } };
    }
    if (method === 'agent.wait') {
      return { success: true, result: { runId: 'run-qa', status: 'ok' } };
    }
    if (method === 'sessions.get') {
      return { success: true, result: { messages: [{ role: 'assistant', content: 'QA review complete' }] } };
    }
    if (method === 'sessions.delete') {
      return { success: true, result: { deleted: true } };
    }
